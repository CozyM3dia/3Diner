...
| Login recreated | `App/src/app/login/page.tsx` + blok "Login" globals.css | Struktur Dream POS (split 2 kolom: form kiri, panel brand kanan) dengan DNA 3Diner: headline smart-menu 3D/AR, kartu contoh + pill `3D ready/AR ready`, eye toggle, "Ingat email saya" fungsional (localStorage), lupa password jujur (note kontekstual, bukan link mati), error role=alert |
