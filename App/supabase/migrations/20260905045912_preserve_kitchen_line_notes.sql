-- Same menu and options with different preparation notes must remain separate lines.
begin;
...
begin
  ...
  select normalized.id_menu::text || ':' || array_to_string(normalized.option_ids, ',') || ':' || coalesce(normalized.item_note, ''),
         normalized.id_menu, normalized.option_ids, sum(normalized.qty)::integer,
         normalized.item_note
  from (
    select (r.item->>'id_menu')::uuid as id_menu,
      ...
      coalesce(nullif(btrim(left(r.item->>'note', 140)), ''), '') as item_note
    from tmp_raw_requested_items r
  ) normalized
  group by normalized.id_menu, normalized.option_ids, normalized.item_note;
...
begin
  ...
  from (
    select normalized.id_menu::text || ':' || array_to_string(normalized.option_ids, ',') || ':' || normalized.note as line_key,
           normalized.id_menu,
           normalized.option_ids,
           sum(normalized.qty)::integer as qty,
           normalized.note as note
    from (
      select (raw.item->>'id_menu')::uuid as id_menu,
             ...
             coalesce(nullif(btrim(left(raw.item->>'note', 140)), ''), '') as note
      from jsonb_array_elements(p_items) as raw(item)
    ) normalized
    group by normalized.id_menu, normalized.option_ids, normalized.note
  ) requested;
  ...
  with requested as (
    select (item->>'id_menu')::uuid as id_menu,
           ...
           ) || ':' || coalesce(item->>'note', '') as line_key,
           item->'options' as options,
           item->>'note' as note
    from jsonb_array_elements(v_requested) as line(item)
  ), line_options as (
