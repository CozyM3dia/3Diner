...
### 6.1 Items / Menu (`/dashboard-v2/items`) — prioritas #1
Template `items.html`: header halaman (`h3` judul + tombol biru "+ Add New" kanan), baris kartu ringkasan status (All Items / Active / Out of Stock dsb dengan icon bulat tinted — angka NYATA dari DB), chips filter kategori (dari distinct `Menus.category`), grid kartu produk: foto (atau placeholder abu), nama (`nama_menu`), kategori, harga (`harga_menu` format `Rp 12.000` via `toLocaleString("id-ID")`), pill status **Live/Offline** nyata dari DB. Detail/edit SEMENTARA arahkan ke editor existing (`src/lib/menu-editor-actions.ts` / halaman menu legacy) — jangan duplikasi editor.
